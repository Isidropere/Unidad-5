...
namespace Practica5_2DesarrolloWebUASD.Controllers
{
    public class EmpleadoesController : Controller
    {
        ...
        public ActionResult Index( string sortOrder)
        {
            ...
            var Empleados = from s in db.Empleadoes where s.EmpleadoID <=5 
                            orderby s.EmpleadoID ascending
                           select s;
            switch (sortOrder)
            {
                
                case "name_desc":
                    Empleados = Empleados.OrderByDescending(s => s.Nombre);
                    break;
                case "Date":
                    Empleados = Empleados.OrderBy(s => s.Fecha_Ingreso);
                    break;
                case "date_desc":
                    Empleados = Empleados.OrderByDescending(s => s.Fecha_Ingreso);
                    break;
                default:
                    Empleados = Empleados.OrderBy(s => s.EmpleadoID);
                    break;
            }
